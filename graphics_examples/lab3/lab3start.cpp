...
GLuint lightmode, lightmodeID; // Variable to pass to the shader and it's ID.
...
//	mat4 projection = perspective(30.0f, 4.f/3.f, 0.1f, 100.0f);	// This strectches objecs with window resize
	...
	mat4 view = lookAt(
		...
		vec3(0+look_inc_x, 1, 0)  // Head is up (set to 0,-1,0 to look upside-down)
	);
	...
	// Passing the lightmode to the shader
...
static void keyCallback(GLFWwindow* window, int key, int s, int action, int mods)
{
	...
	if (key == 'P') look_inc_x += 0.05f;
	if (key == 'O') look_inc_x -= 0.05f;
